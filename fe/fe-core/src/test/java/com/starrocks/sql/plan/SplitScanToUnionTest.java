...
class SplitScanToUnionTest extends DistributedEnvPlanTestBase {
    ...
    private static Stream<Arguments> testSplitUnionSqls() {
        ...
        sql = "select * from orders where (O_CUSTKEY = abs(1) or O_ORDERKEY = 123 or " +
                "O_ORDERDATE = str_to_date('2014-12-21', '%Y-%m')) and O_CLERK > O_ORDERPRIORITY";
        arguments = Arguments.of(sql, ImmutableList.of("UNION",
                "PREDICATES: 1: O_ORDERKEY = 123, 7: O_CLERK > 6: O_ORDERPRIORITY",
                "PREDICATES: 2: O_CUSTKEY = CAST(abs(1) AS INT), 1: O_ORDERKEY != 123, 7: O_CLERK > 6: O_ORDERPRIORITY",
                "PREDICATES: 5: O_ORDERDATE = str_to_date('2014-12-21', '%Y-%m'), 1: O_ORDERKEY != 123, " +
